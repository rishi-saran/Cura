...
// NOTE: No FCM register/getToken here. That is handled in firebase-config.js
...
function openModal(modalId) {
  if (!currentEditId) {
    document.querySelector(`#${modalId}`).querySelectorAll('input').forEach(i => i.value = '');
  }
...
async function saveReminder(type) {
  ...
  const dataToSave = { userId: djangoUserId };
  ...
  try {
    ...
    if (Object.values(dataToSave).some(v => v === "")) return alert("Please fill out all fields.");
...
async function recordAction(type, docId, action) {
  ...
  try {
    ...
    console.log(`Reminder ${docId} marked as ${action}.`);
  } catch (error) {
...
function loadDynamicData(userId) {
        ...
        medContainer.innerHTML += `
          <div class="template-card ${s.status === 'Done' ? 'done-card' : ''}">
            <div class="card-actions">
              <button class="icon-btn" aria-label="delete" onclick="deleteReminder('medication', '${doc.id}')">🗑</button>
              <button class="icon-btn" aria-label="edit" onclick="openEditModal('medication', '${doc.id}')">✏️</button>
            </div>
            <div class="card-body">
    ...
    db.collection('appointmentSchedules').where('userId', '==', userId).orderBy('datetime', 'desc').onSnapshot(snapshot => {
      ...
      snapshot.forEach(doc => {
        ...
        apptContainer.innerHTML += `
          <div class="template-card ${s.status === 'Done' ? 'done-card' : ''}">
            <div class="card-actions">
              <button class="icon-btn" aria-label="delete" onclick="deleteReminder('appointment', '${doc.id}')">🗑</button>
              <button class="icon-btn" aria-label="edit" onclick="openEditModal('appointment', '${doc.id}')">✏️</button>
            </div>
            <div class="card-body">
              <h2 class="card-title">${s.doctor}</h2>
              <div class="card-details">
                <p><strong>Date:</strong> ${d.toDateString()}</p>
  ...
  if (activityContainer) {
    db.collection('activitySchedules').where('userId', '==', userId).orderBy('time').onSnapshot(snapshot => {
      ...
      snapshot.forEach(doc => {
        ...
        activityContainer.innerHTML += `
          <div class="template-card ${s.status === 'Done' ? 'done-card' : ''}">
            <div class="card-actions">
              <button class="icon-btn" aria-label="delete" onclick="deleteReminder('activity', '${doc.id}')">🗑</button>
              <button class="icon-btn" aria-label="edit" onclick="openEditModal('activity', '${doc.id}')">✏️</button>
            </div>
            <div class="card-body">
    ...
    }, e => handleError(e, activityContainer));
  }
}
